...
eight real donor spectra sampled uniformly from the same split side without
conditioning on the centre label (resampled once if the donor's core equals
the centre's, so the donor distribution depends weakly on the centre's
core). In the preprocessed space, donor $d$ receives the constructed cue
...
are reported separately and were not used for any choice. Retraining
follows the shared protocol of Appendix~\ref{app:exp2}: a fresh entire
patch head ($M=32$) trained for $20{,}000$ steps on new context-random
patches around the same training centres from each frozen $L^\ast$ encoder
(random initialization, $\kappa=1$, $\kappa=1/256$), evaluated at its final
state on the paired validation and test families.

\subsection{Projection of the net encoder update}
For each saved encoder pair $(W_0,W^\ast)$ at $L^\ast$ we report
$R_C=\|(W^\ast-W_0)C^\top\|_F^2/\|W^\ast-W_0\|_F^2$, where the rows of
$C\in\R^{8\times942}$ are the eight orthonormal constructed-cue directions
in the preprocessed coordinates of the regime ($v_1,\dots,v_8$ in the
standardized regime; the first eight whitened coordinates in the whitened
regimes). $R_C$ describes where the net adaptation lies; it is not a
pathwise gradient decomposition, and a high value is not by itself a
suppression diagnostic: in the standardized regime the uninformative-context
comparator also adapts almost entirely within these directions, which are
the top principal directions there. Table~\ref{tab:exp3_proj} gives the
range over the registered seeds
(\texttt{results/exp3/update\_projections.csv},
\texttt{code/experiments/update\_projections.py}).

\begin{table}[h]
\centering\small
\caption{Fraction $R_C$ of the squared net encoder update $W^\ast-W_0$ at
$L^\ast$ lying in the eight constructed-cue directions; range over runs
(seeds $0$--$2$; \texttt{sp} over its saved widths, \texttt{headlr} over
$\kappa\in\{1,1/16,1/256\}$, \texttt{ctxfree} over its widths).}
\label{tab:exp3_proj}
\begin{tabular}{llll}
\toprule
regime & \texttt{sp} & \texttt{headlr} & \texttt{ctxfree} \\
\midrule
high accessibility (standardized) & 0.99988--0.99996 & 0.99990--0.99995 & 0.99668--0.99779 \\
low accessibility, $\gamma=30$ & 0.99995--0.99998 & 0.99995--0.99997 & 0.88118--0.89806 \\
low accessibility, $\gamma=10$ & 0.99968--0.99978 & 0.99961--0.99978 & 0.89131--0.90217 \\
\bottomrule
\end{tabular}
\end{table}
...
Section~\ref{sec:real} keep their three-seed values. The added seeds agree on each of the following contrasts:
...
\IfFileExists{sections_iclr_v2/appendix_exp3_tables.tex}{\input{sections_iclr_v2/appendix_exp3_tables}}{\emph{[tables pending]}}
...
retraining procedures) are run on the other four patient-level folds of the
...
When all four added folds are complete, tables in this subsection will give the
fold-to-fold variation of the reported contrasts as mean and range, and any contrast
whose sign is not the same in all five folds will be named as fold-dependent; at the
time of this build the fold runs were in progress, and no fold result is cited
anywhere in the paper.
\IfFileExists{sections_iclr_v2/appendix_folds_tables.tex}{\input{sections_iclr_v2/appendix_folds_tables}}{}

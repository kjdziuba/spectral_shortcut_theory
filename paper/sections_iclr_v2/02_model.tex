...
the primary experiments, with a two-layer ReLU encoder extension in
Section~\ref{sec:synthetic}; the spatial model is a replicated readout in
the theorem, a ReLU convolutional head in the synthetic experiment and a
ReLU patch head in the real-spectrum experiment. Training minimizes the
...
encoder must discover: in the constructed low-accessibility settings a
randomly initialized encoder attenuates it. The
...
accuracy on calibration data (matched signal-to-noise synthetically; a
training-side estimate on real spectra), so that a
preference for one cue is not simply a preference for the more predictive
one. The theorem allows any $(a_0,v_0)$; the synthetic experiment selects
a small initial spectral contribution and an accessible contextual one
and measures an empirical accessibility analogue, not the theorem's
initial margin coefficients (Section~\ref{sec:synthetic}); the
real-spectrum experiment adds a high-accessibility regime.
...
trained comparator, without assuming a zero-error reference; the probe
is linear accessibility of the spectral cue on clean spectral inputs.

\paragraph{Test conditions.}
Every evaluation family is generated from the same labels and noise as
\emph{iid} (context agrees with the label), \emph{reversed} (the assigned
context carries the opposite label) and \emph{context-random} (an
independent label: context present but uninformative). In the synthetic
experiment the intervention changes the contextual field at every pixel
while the target's spectral component, label and noise are shared; in the
real-spectrum experiment each centre spectrum is held fixed. Reversal
measures reliance; context-random measures what the predictor does from
the spectrum in the presence of context.

\begin{figure}[t]
\centering
\IfFileExists{figures/fig1_model.pdf}{\includegraphics[width=0.84\linewidth]{figures/fig1_model.pdf}}{\fbox{\rule{0pt}{1.6in}\rule{0.97\linewidth}{0pt}}}
...
neighbourhood (a $5\times5$ receptive field in Experiment 2; one
nine-spectrum patch in Experiment 3). The contextual cue lies in the
eight neighbours along large-amplitude directions that a random encoder
passes; the spectral cue lies in the pixel's own spectrum along a
small-amplitude direction that, in the low-accessibility settings,
requires encoder alignment. Evaluation under iid, reversed and
context-random contexts measures reliance and failure.}
\label{fig:model}
\end{figure}

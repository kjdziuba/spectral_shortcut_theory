...
universal one. The frozen comparator is evaluated at the joint path's
elapsed times through $T_m$. Part (c) removes the width dependence
...
the competition parameter the experiments manipulate. For
$(a_0,v_0)=(0,1)$ the scalar-logit GGN block ratio
$\mathcal D_{\rm curv}(0)$ equals $M$ (generally $Mv_0^2$) and
$\Psi(m)/(M+1+2m^2)\le T_m\le\Psi(m)/(M+1)$ with $\Psi(m)=m+e^m-1$
...
$1.3\times10^{-11}$, with no bound violated, and the Gaussian sweep
approaches $\Phi(1)$ ($0.8207$, $0.8380$, $0.8397$ at $M=64$, $1024$,
$16384$; Appendix~\ref{app:verification}): a check of the algebra, not of
the claim beyond the model.

\begin{figure}[t]
\centering
\IfFileExists{figures/fig2_mechanism.pdf}{\includegraphics[width=0.84\linewidth]{figures/fig2_mechanism.pdf}}{\fbox{\rule{0pt}{2.1in}\rule{0.97\linewidth}{0pt}}}
...
against width $M$ for four initializations $(a_0,v_0)$ (solid), the
bound $1/(1+Mv_0^2)$ (dashed) and the normalized readout of part (c)
(grey, flat at the $M=1$ value). (b) Joint-minus-frozen margin gap with
the bound $C_0/(p_0Mv_0^2)$ (dashed). (c) Expected reversal error over
$(a_0,v_0)\sim\mathcal N(0,I_2)$ against $M$ for $m=2$ with the limit
$\Phi(m/2)$; the spectral-only comparator has error zero.}
\label{fig:mechanism}
\end{figure}

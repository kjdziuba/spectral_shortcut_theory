...
\item \textbf{Synthetic intervention.} In a ReLU convolutional head over a
  ...
  Retraining the head on decorrelated context recovers shifted accuracy
  of $0.75$--$0.84$ after the slow head and $0.54$--$0.62$ after the fast
  one, no more than from random encoders.
\item \textbf{Real spectra.} With measured centre spectra in constructed
  ...
  with a rate response that grows as the cue amplitude falls, and more
  than $99.9\%$ of the net encoder update lies in the constructed-cue
  span. Where the spectral cue is accessible, retraining the head recovers
  $0.90$--$0.96$ shifted accuracy from every encoder, a random one
  included: the failure was in the use of the spectrum, not its
  availability. With natural neighbourhoods, on tissue and on a public
  hyperspectral scene, no competition arises (Appendices~\ref{app:exp4}
  and~\ref{app:pavia}).
...
recover, measured at matched fit against a control. In infrared
histology, insensitivity to a learned sixteen-feature spectral bottleneck
has been read as showing that tissue classification needs only a small
set of spectral features \citep{oleary2026spatial}, and CNN classifiers
are reported insensitive to the compression method
\citep{mueller2023dimred}.
Appendix~\ref{app:exp4} tests that reading with natural neighbourhoods: a
learned twelve-dimensional bottleneck loses nothing, sixteen principal
components lose $0.07$--$0.11$ macro-F1 per pixel and $0.04$ for the
spatial model, and the jointly trained encoder learns the class signal
exactly as it does without informative context; compressibility is not
redundancy, and a natural $3\times3$ neighbourhood, being the same class
as its centre, is not the competing cue of the theorem. The mathematics
...
learning outcomes rather than curvature, whose width dependence is
normalization-driven \citep{karakida2019universal,vanlaarhoven2017l2}

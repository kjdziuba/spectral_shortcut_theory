...
\ref{tab:exp3_ready_paviau}).} All $78$ runs reach $L^\ast$. In the low-accessibility
regime the outcome is that of the tissue experiment: every informative-context arm
leaves the encoder's probe at its initial value (gain $-0.002$ to $+0.002$ at
$\gamma=30$, at most $+0.008$ at $\gamma=10$, from $0.707$) and fits within $15$ to
$420$ steps, while the uninformative-context comparator raises the probe by $0.22$ to
$0.24$ and needs about $9{,}000$ steps; reversal accuracy is $0.05$ to $0.12$ against
$0.84$ to $0.90$, and context-random accuracy is at chance. The rate response is weaker
than on tissue: at $\gamma=30$ the within-seed probe change from $\kappa=1$ to $1/256$
is $+0.023$, $-0.009$, $-0.006$ (monotone in one seed of three), and at $\gamma=10$ it is
$+0.073$, $-0.001$, $+0.005$ (two of three); only the seed whose random encoder started
least accessible ($0.664$ against $0.703$ and $0.755$) responds, and reversal accuracy
is unchanged. The registered rate prediction is therefore not met at $\gamma=30$ and
partially met at $\gamma=10$ on this scene. In the high-accessibility regime the probe
is $0.91$ to $0.92$ throughout and the informative-context arms still fail under
reversal ($0.19$ to $0.26$ against $0.85$ to $0.88$), as on tissue. Retraining
(Table~\ref{tab:exp3_recovery_paviau}) also matches tissue: in the high-accessibility
regime a fresh head recovers reversal accuracy $0.885$, $0.887$ and $0.907$ from the
random, the $\kappa=1$ and the $\kappa=1/256$ encoders, the ordering holding in every
seed with a small margin ($+0.014$ to $+0.028$), while in the low-accessibility regime
no encoder yields more than the random one at $\gamma=30$ ($0.65$ to $0.66$) and only
the responsive seed gains at $\gamma=10$ ($+0.060$, $-0.032$, $+0.001$). On this scene the
mechanism's suppression and reliance replicate in full; its rate response and the
recoverability it governs replicate where the cue is accessible and only partially
where it is not.
...
\ref{tab:exp4_recovery_paviau}).} The natural neighbourhoods are as homogeneous as on
tissue (no patch straddles two labelled classes; $88\%$ of neighbours share the centre
label), and the outcome matches the tissue result on every registered item. Natural
context is worth $0.25$ macro-F1 in distribution ($0.840$ against $0.594$ for the
shuffled comparator), and the natural-context model collapses to $0.144$ under
context-random evaluation (nine-class chance $0.11$). Its encoder is not starved: the
probe is $0.732$ against the comparator's $0.722$, from $0.715$ at initialization,
this scene being in the high-accessibility regime under standardization. A fresh head
retrained on context-random patches recovers $0.629$ from the natural-context encoder
against $0.627$ for the comparator and $0.559$ from a random encoder, so the collapse is
the head's use of same-class neighbours (P9, head-level). The learned twelve-dimensional
bottleneck equals no bottleneck ($0.840$ against $0.836$ in distribution; $0.627$
against $0.654$ for the comparators under context-random evaluation; P11). Two
limitations of the head family are visible here: the $M=32$ ReLU head trained by plain
gradient descent is a weak nine-class per-pixel classifier (the comparator reaches
$0.627$ where the per-pixel MLP reaches $0.911$, and the no-bottleneck comparator
$0.654$), so the second clause of P8 fails, and the slow-head arm ($\kappa=1/256$)
does not reach any matched-fit threshold within the budget (training loss $0.84$),
so the rate prediction is not evaluable at matched fit and is not met at budget end.

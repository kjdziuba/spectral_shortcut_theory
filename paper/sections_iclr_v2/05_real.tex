...
We test contextual competition with measured centre spectra and their
recorded labels, adding a synthetic class-associated intensity pattern
along training-estimated principal directions in the neighbourhood: does
the mechanism operate when the spectral cue is a real, high-dimensional
class contrast? Natural neighbourhoods are treated in
...
whitening (training patients) reduces the random-encoder probe to $0.54$
while the discriminant stays at $0.944$. We therefore run
...
Each patch is a real centre with eight real donor spectra drawn without
conditioning on the centre label from the same split side; donor $d$
...
natural variation ($3\sqrt{\lambda_d}$ standardized; $30$ whitened, with
a $\gamma=10$ variant declared before its runs). $\tau$ is
...
(Table~\ref{tab:exp3_ready}): a large probe deficit is not necessary for
failure.
...
mechanism: the projection is not a pathwise decomposition, and the random
head, unlike the theorem's zero-sum readout, lets contextual learning
proceed through the encoder while the head barely moves.
...
Retraining separates the regimes further (Table~\ref{tab:exp3_recovery}).
...
original failure coexisted with spectral information that the specified
retraining could use. Under this procedure the nine $\gamma=30$ encoders
yield $0.45$--$0.56$; at $\gamma=10$ the slow-head encoders yield
$0.55$--$0.70$ against $0.48$--$0.60$ after the fast head (paired gain
$+0.07$, $+0.10$, $+0.07$).

\paragraph{Reading.}
With measured spectra the outcome matches the synthetic experiment on
reliance: at every width and rate the original predictor uses informative
constructed context, with excess shifted risk of $0.83$--$0.84$ over the
comparator. On
adaptation it adds a scope condition: how much of the contrast the
encoder learns before the fit depends on the head's relative speed and
the cue's amplitude, which are not interchangeable here. On the public
Pavia University scene the constructed-context experiment reproduces
suppression and reliance, while its low-accessibility rate and recovery
effects are inconsistent across seeds (Appendix~\ref{app:pavia}). In
natural $3\times3$ neighbourhoods, on tissue and on that scene,
informative- and shuffled-context training give similar encoder probes
and comparator-level recovery: reliance without the accessibility
deficit seen with constructed context, which neither identifies why
suppression was absent nor shows that natural context cannot produce it
(Appendix~\ref{app:exp4}).

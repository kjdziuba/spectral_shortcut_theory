...
evades most of them instructively: its classifier head has a fixed input
dimension, so the width-linear floor has no premise; its inputs are
effectively rank one, so the top-eigenvalue ratio is uninformative and a
directional picture must replace it; the width growth of its downstream
curvature is a normalization gain that a function-preserving rescaling moves
...
initialization curvature predicts only an early phase. The paper's practical
content is the set of measurements one must make before reading a shortcut
into a curvature ratio or a training curve, not a recipe.

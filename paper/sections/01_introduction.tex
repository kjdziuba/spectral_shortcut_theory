...
\begin{itemize}
    \item \textbf{Theorem~\ref{thm:hessian} (Capacity-Induced Module
          ...
          (equivalently, as $\sqrt{\Cg/\Cf}$ for fixed-depth
          architectures): as the spatial module widens, curvature
          concentrates in it while the spectral block's curvature
          stays capped by a width-independent constant. The proof
          combines the mean-field FIM scaling of Karakida et
          al.~\cite{karakida2019universal} with an operator-norm
          bound on the spectral block.

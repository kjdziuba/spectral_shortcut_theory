...
the realized disparity correspondingly small. Whether an input
transformation moves the \emph{realized} disparity, however, depends
on the architecture between $f_\thetap$ and the loss: internal
normalization layers can absorb input shifts entirely (their
Jacobians annihilate constant directions), in which case the
effective regressor is already centered and the lever acts through
the normalization choice, not the preprocessing. We measure exactly
this on the production architecture in Section~\ref{sec:real}.

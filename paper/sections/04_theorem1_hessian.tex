...
an MLP). Under
Assumptions~\ref{ass:linear}--\ref{ass:inputlip} and the head
hypothesis of Lemma~\ref{lem:phi_scaling} below (a dense classifier
head over a penultimate feature vector of dimension $\Theta(M)$ with
concentrated squared norm at initialization), there exist
...
initialization. Equivalently, for
...
MLP), let $N_{cls} \geq 2$, and suppose the classifier head of
$g_\phip$ is a dense layer applied to a penultimate feature vector
$h \in \mathbb{R}^{M_h}$ with $M_h = \Theta(M)$ whose squared norm
concentrates at initialization: $\norm{h}^2 \geq q_h M_h$ with
probability at least $1 - \delta_h$ over the random initialization,
for constants $q_h > 0$ and $\delta_h < 1/2$ independent of $M$
(the \emph{head hypothesis}; satisfied by Kaiming Gaussian
initialization---see the supplement). Then, under
Assumptions~\ref{ass:reg}--\ref{ass:subg}, the largest eigenvalue of
the spatial Gauss-Newton block satisfies
\[
    \lambda_{\max}(J_\phip^\top J_\phip)
    \;=\; \Omega(M)
    \qquad \parbox{16em}{\centering in expectation over random Kaiming
...
(Section~\ref{sec:proof_thm1}) by an explicit witness: a head-weight
direction aligned with the penultimate feature vector $h(x)$ of a
single classified pixel. At variance-scaled initialization
$\norm{h}^2 = \Theta(M)$, and passing the softmax inner matrix costs
only a constant, forcing $\lambda_{\max} = \Omega(M)$. The

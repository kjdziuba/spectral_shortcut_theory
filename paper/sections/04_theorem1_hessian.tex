...
\begin{remark}
Lemma~\ref{lem:phi_scaling} rests on the mean-field analysis of
Karakida et al.~\cite{karakida2019universal}, who show that the
leading eigenvalue of the Fisher Information Matrix of a
fully connected network at random Gaussian initialization grows
linearly in the network \emph{width}. The scaling variable is width,
not parameter count: for fixed-depth architectures with
$\Cg = \Theta(M^2)$, the equivalent parameter-count statement is
$\lambda_{\max} = \Omega(\sqrt{\Cg})$. Karakida's derivation covers
fully connected architectures under squared loss; we adopt the
width-linear scaling as the asymptotic prediction for the CNN and ViT
families as well, and verify empirically in
Section~\ref{sec:experiments} that the measured scaling is consistent
with it across all three architectures.
\end{remark}

...
$h_n \in \mathbb{R}^{M_h}$ with $M_h = \Theta(M)$ whose mean Gram
$S_h := N^{-1}\sum_n h_n h_n^\top$ satisfies
\[
    \lambda_{\max}(S_h) \;\geq\; q_h M_h
    \quad\text{with probability } \geq 1 - \delta_h
\]
over the random initialization, for constants $q_h > 0$ and
$\delta_h < 1/4$ independent of $M$ (the \emph{head hypothesis}:
the features carry a common direction of $\Theta(M)$ energy across
inputs; it holds at Kaiming initialization for any nonnegative or
nonzero-mean activation, and coincides with the condition
$\kappa_2 > 0$ under which Karakida et al.'s width-linear law
itself holds---see the supplement). Then, under

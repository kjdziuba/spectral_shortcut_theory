...
$J_\bullet^{\mathrm{res}}$. All Gram matrices, and $\Sigma_X$
in~\eqref{eq:sigma_x}, use this same $1/N$ normalization, so no
constant in this paper depends on the dataset size.
...
\label{eq:chain}
    \nabla_\thetap \loss
    = \underbrace{\frac{\partial \loss}{\partial \hat{y}}}_{\text{residual } r}
...
\end{equation}
where the softmax enters through the first factor, $\partial \loss /
\partial \hat{y} = r$ (the residual \emph{is} the softmax-weighted
loss gradient; the factor $H_\tau$ appears explicitly only when
differentiating $r$ itself, as in
$J^{\mathrm{res}}$ above and in the Gauss-Newton blocks below).

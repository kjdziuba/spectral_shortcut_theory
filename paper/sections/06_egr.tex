...
training pathology that is invisible to \emph{single-run} loss-curve
monitoring. The failure is counterfactual: a jointly trained model
converges to an apparently healthy solution---training loss down,
validation metrics respectable, every curve shaped like a normal
run---that is nonetheless worse than what the \emph{same
architecture} achieves with its spectral module frozen. Nothing in
the run's own curves flags this, because the deficit is visible only
against a baseline the practitioner has not trained. One discovers
it by training the frozen counterpart (twice the compute, and only
if one thinks to try), or not at all.
...
and~\ref{ass:residual}, with the trajectory contained in
$\Phi_{\mathrm{reg}} \times \mathcal{Z}_{\mathrm{reg}}$ up to a
horizon $T$---we have, for every $t \leq T$,
\[
    \norm{\nabla_\thetap \loss(t)}
...
\]
with $B_T$ as in~\eqref{eq:bt}, $C_\thetap$ the cap of
Lemma~\ref{lem:opcap}, and $C, \mu$ the constants of
Assumption~\ref{ass:residual}.
\end{proposition}

\begin{proof}
Identical to~\eqref{eq:theta_grad_bound}:
$\norm{\nabla_\thetap \loss} = \norm{J_\thetap^\top r} \leq
\norm{J_\thetap}_{\mathrm{op}} \norm{r} \leq B_T \norm{r}$ for
$t \leq T$, and Assumption~\ref{ass:residual} bounds
$\norm{r(t)}$.
\end{proof}

\begin{remark}
The horizon restriction is not a technicality inherited for
tidiness: Remark~\ref{rem:containment} notes that in the
unregularized separable regime---the regime
Assumption~\ref{ass:residual} models---a cap uniform over all
$t \in [0, \infty)$ cannot be assumed. In practice $T$ is the
training run's length, which is exactly the horizon the diagnostic
operates over.
\end{remark}
...
The absolute decay of the spectral gradient
(Proposition~\ref{prop:egr_monotonic}), combined with the empirical
behavior of the ratio characterized in
Section~\ref{sec:egr-empirical}, motivates a practical criterion for
detecting the onset of gradient starvation. We emphasize its status:
the threshold rule is an empirically calibrated heuristic that the
theory motivates but does not derive
(Remark~\ref{rem:egr_ratio_caveat}).
...
value we use throughout Section~\ref{sec:experiments}.
\end{remark}
